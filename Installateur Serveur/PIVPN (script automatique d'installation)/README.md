PIVPN 
--
PiVPN est un utilitaire utile de OpenVPN qui permet la création d'utilisateur et la sécurisation de la connexion via un login/mdp (ce qui peut être securitaire mais peut poser problème si on ne met pas dans les fichiers clients la connexion automatique via un script ou l'autorun de OpenVPN).
